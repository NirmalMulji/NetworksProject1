...
def checkValidity2(recvMessage):
    ...
    # checks if Lab is 1
    if not (recvMessage[1] == 1):
        print "Syntax Error: lab is not 1"
        return 2 + (1 << 15)

        # checkLab = True

    # checks if version is 7
    if not (recvMessage[2] == 7):
        print "syntax error: version is not 7"
        return 2 + (1 << 15)
    ...
    # verifies that "transaction outcome" bit of the result is not "1"
    if not ((recvMessage[6] >> 15) < 1):
        print "syntax error: transaction outcome bit is 1"
        return 2 + (1 << 15)
        # checkResult = True
